...
class PerceiverAttentionCA(nn.Module):
    def __init__(self, kv_dim, dim=3072, dim_head=1024, heads=33):
        super().__init__()
    ...
    def forward(self, x, latents,audio_scale):
        """
        Args:
...
class AudioAttentionFA(nn.Module):
    def __init__(self, kv_dim, dim=3072, dim_head=1024, heads=33):
        super().__init__()
        self.scale = dim_head ** -0.5
        self.dim_head = dim_head
    ...
    def forward(self, x, latents,audio_scale,bbox):
        """
        Args:
            x (torch.Tensor): image features
                shape (b, t, aa, D)
        ...
        out = weight @ v
        out=self.to_out(out)
        #outputs=res.view(x.shape[0],-1,5120)+out.view(x.shape[0],-1,5120)*bbox.unsqueeze(2)
        outputs=res+out*bbox.unsqueeze(3)
        #outputs=res+out
        # out = out.permute(0, 2, 1, 3)
        return outputs
